...
exports.addProductToCart = asyncHandler(async (req, res, next) => {
  ...
  if (!cart) {
    ...
    } else cart.cartItems.push({ product: productId, color, price: product.price });
  }
...
exports.updateCartItemQuantity = asyncHandler(async (req, res, next) => {
  ...
  const cart = await Cart.findOne({ user: req.user._id });
  if (!cart) return next(new ApiError(`there is no cart for user ${req.user._id}`, 404));
  const itemIndex = cart.cartItems.findIndex((item) => item._id.toString() === req.params.itemId);
  if (itemIndex > -1) {
    const cartItem = cart.cartItems[itemIndex];
    cartItem.quantity = quantity;
    cart.cartItems[itemIndex] = cartItem;
  } else return next(new ApiError(`there is no item for this id :${req.params.itemId}`, 404));
